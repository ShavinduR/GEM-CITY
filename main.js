...
(function () {
    ...
    const CACHE_KEY = 'facet_exchange_rates';
    const CACHE_EXPIRY = 24 * 60 * 60 * 1000; // 24 hours

    const currencyConfig = {
        LKR: { symbol: 'LKR', decimals: 0 },
    ...
    async function updatePrices(targetCurrency) {
        ...
        document.querySelectorAll('.dynamic-price').forEach(el => {
            const basePrices = el.getAttribute('data-base-price').split(',');
            const converted = basePrices.map(p => {
                const convertedVal = parseFloat(p) * rate;
                return formatPrice(convertedVal, targetCurrency);
            });
            el.innerText = converted.join(' - ');
        });
    }

    if (currencySelector) {
        // Load saved currency or default to LKR
        const savedCurrency = localStorage.getItem('selected_currency') || 'LKR';
